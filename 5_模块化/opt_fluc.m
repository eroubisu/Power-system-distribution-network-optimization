function data3 = opt_fluc(data1, data2)
    mpc = data1;
    %% 1.设参
    T = 24;
    %读取数据
    ...
    num_load_sd = size(Bus_load_sd, 1);
    %目标函数系数
    ...
    areaC = mpc.areaC;
    areaD = mpc.areaD;
    RES_cspt_all = mean(sum(data2.UP_P_wt + data2.UP_Q_wt + data2.UP_P_pv)); %计划新能源消纳量
    RES_cspt_A = mean(sum(data2.P_wt(areaA, :) + data2.Q_wt(areaA, :) + data2.P_pv(areaA, :)));
    RES_cspt_B = mean(sum(data2.P_wt(areaB, :) + data2.Q_wt(areaB, :) + data2.P_pv(areaB, :)));
    RES_cspt_C = mean(sum(data2.P_wt(areaC, :) + data2.Q_wt(areaC, :) + data2.P_pv(areaC, :)));
    RES_cspt_D = mean(sum(data2.P_wt(areaD, :) + data2.Q_wt(areaD, :) + data2.P_pv(areaD, :)));
    ...
    %FCE
    P_FCE = P_Gen - data2.P_Gen;
    UP_P_FCE = [abs(P_Gen(69, :) - data2.P_Gen(69, :)) + abs(P_Line(27, :) - data2.P_Line(27, :)) + abs(P_Line(36, :) - data2.P_Line(36, :)); abs(P_Line(27, :) - data2.P_Line(27, :)); abs(P_Line(36, :) - data2.P_Line(36, :)); abs(P_Line(10, :) - data2.P_Line(10, :))]
    %% 9.波动模型设约束
    C_fluc = [];
    %% 9.1需求响应约束
    C_fluc = [C_fluc, 0 <= UP_P_load_sd <= 0.8 * P_Load(mpc.load_sd(:, 1), :)];
    for i = 1:num_load_sd
    ...
    %% 9.3风电+光伏约束
    C_fluc = [C_fluc, 0 <= UP_P_wt, UP_P_wt <= P_WT];
    C_fluc = [C_fluc, 0 <= UP_Q_wt, UP_Q_wt <= Q_WT];
    for i = 1:num_wt
        P_wt = [P_wt(1:mpc.P_WT(i, 1) - 1, :); UP_P_wt(i, :); P_wt(mpc.P_WT(i, 1):end, :)];
        Q_wt = [Q_wt(1:mpc.Q_WT(i, 1) - 1, :); UP_Q_wt(i, :); Q_wt(mpc.Q_WT(i, 1):end, :)];
    end
    C_fluc = [C_fluc, 0 <= UP_P_pv, UP_P_pv <= P_PV];
    for i = 1:num_pv
        P_pv = [P_pv(1:mpc.P_PV(i, 1) - 1, :); UP_P_pv(i, :); P_pv(mpc.P_PV(i, 1):end, :)];
    end
    C_fluc = [C_fluc, mean(sum(UP_P_wt + UP_Q_wt + UP_P_pv)) >= min(mean(sum(P_WT + Q_WT + P_PV)), RES_cspt_all)]
    %% 9.4潮流约束
    P_bus_in = -upstream * P_Line + upstream * (I .* (R * ones(1, T))) + dnstream * P_Line; %节点注入有功
    Q_bus_in = -upstream * Q_Line + upstream * (I .* (X * ones(1, T))) + dnstream * Q_Line; %节点注入无功
    C_fluc = [C_fluc, P_bus_in + P_Load - P_load_sd - P_Gen - P_wt - P_pv - P_ess_dch + P_ess_ch == 0];
    C_fluc = [C_fluc, Q_bus_in + Q_Load - Q_wt - Q_Gen == 0];
    %欧姆定律约束
    ...
    f_sd_fluc = mean(sum(UP_P_load_sd)) / mean(sum(P_Load(mpc.load_sd(:, 1), :))); %负荷削减量
    ...
    %% 12.传出参数
    data3 = struct;
    data3.UP_P_wt = value(UP_P_wt);
    data3.UP_Q_wt = value(UP_Q_wt);
    data3.UP_P_pv = value(UP_P_pv);
    data3.P_wt = value(P_wt);
    data3.Q_wt = value(Q_wt);
    data3.P_pv = value(P_pv);
    data3.UP_P_ess_dch = value(UP_P_ess_dch);
    data3.UP_P_ess_ch = value(UP_P_ess_ch);
    data3.SOC_ess = value(SOC_ess);
    data3.P_Gen = value(P_Gen);
    data3.Q_Gen = value(Q_Gen);
    data3.P_Line = value(P_Line);
end
